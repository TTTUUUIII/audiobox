...
public class AudioFilter {
    ...
    public AudioFilter(@FilterChannel int channelFilter, @FilterMode int filterModel) {
    ...
    public short[] filter(short[] rawData, int offset) {
        if (filterModel == FILTER_MODEL_FILL) {
            fill(rawData, offset);
            return rawData;
        } else return clip(rawData, offset);
    }

    private void fill(short[] rawData, int offset) {
        for (int i = offset + filterChannelId; i < rawData.length; i += 2) {
            rawData[i] = 0;
        }
    }

    private short[] clip(short[] rawData, int offset) {
        short[] result = new short[(int)(rawData.length / 2)];
